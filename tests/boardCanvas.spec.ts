...
for (const viewport of [
  ...
  test(`hero drop uses the map origin at ${viewport.width}x${viewport.height}`, async ({
    page,
  }) => {
...
test("selection and late portraits preserve a drag through board updates", async ({
  page,
}) => {
  ...
  await expect
    .poll(() =>
      page.evaluate(() => window.boardHarness.snapshot.selectedTokenId),
    )
    .toBe("ally-strange");
  releaseImages();
  ...
  await page.mouse.up();
  await expect
    .poll(() => page.evaluate(() => window.boardHarness.moves))
    .toEqual([{ x: 300, y: 280 }]);
...
test("drop coordinates follow resize and map changes", async ({ page }) => {
  await page.goto("/tests/fixtures/board.html");
  expect(
    await page.evaluate(() => window.boardHarness.board.toBoardPoint(120, 120)),
  ).toEqual({ x: 200, y: 200 });
  ...
  await expect
    .poll(() =>
      page.evaluate(() => window.boardHarness.board.toBoardPoint(120, 120)),
    )
    .toEqual({ x: 400, y: 400 });
  await page.evaluate(() => {
...
test("destroy ignores pending images and repeated cleanup", async ({
  page,
}) => {
  const errors: string[] = [];
...
test("late map images cannot replace the current map or selection", async ({
  ...
  const oldMapReady = new Promise<void>((resolve) => {
    releaseMap = resolve;
  });
  ...
  const ringPixel = await page
    ...
    .evaluate((canvas) => {
      ...
      const context = canvas.getContext("2d");
      if (!context) throw new Error("Missing context");
      // At half scale, the selected ring spans x=109..112; sample inside it.
