...
async function boardPoint(page: Page, x: number, y: number): Promise<{ x: number; y: number }> {
...
async function dropHero(page: Page, heroId: string, x: number, y: number): Promise<void> {
  ...
  await page.locator('.stage-host canvas').first().dispatchEvent('drop', {
    dataTransfer, clientX: point.x, clientY: point.y
  });
  await dataTransfer.dispose();
}

async function clickToken(page: Page, x: number, y: number, button: 'left' | 'right' = 'left'): Promise<void> {
...
test('placement, removal, Clear and Reset restore in history order', async ({ page }) => {
  await page.goto('/');
...
test('one drag is one edit; selection and unchanged edits preserve redo', async ({ page }) => {
  await page.goto('/');
  const undo = page.getByRole('button', { name: 'Undo', exact: true });
  const redo = page.getByRole('button', { name: 'Redo', exact: true });
  ...
  await page.getByRole('button', { name: 'Clear', exact: true }).click();
  await undo.click();
  await expect(page.getByRole('button', { name: /^Allies/ })).toHaveText('Allies 3');
  await expect(undo).toBeDisabled();
});

test('map changes restore clamped positions in the same step', async ({ page }) => {
  await page.goto('/');
...
for (const modifier of ['Control', 'Meta']) {
  test(`${modifier} shortcuts preserve text editing and view state`, async ({ page }) => {
    await page.goto('/');
    await dropHero(page, 'hulk', 500, 250);
    ...
    await expect(page.getByRole('button', { name: /^Allies/ })).toHaveText('Allies 4');
    ...
    const prevented = await page.evaluate((key) => {
      ...
      const results = [input, textarea, child].flatMap((target) => [false, true].map((shiftKey) => {
        const event = new KeyboardEvent('keydown', {
          key: 'z', ctrlKey: key === 'Control', metaKey: key === 'Meta', shiftKey,
    ...
    await expect(page.getByRole('button', { name: /^Opponents/ })).toHaveAttribute('aria-pressed', 'true');
...
test('undo during a drag cancels the gesture without adding an edit', async ({ page }) => {
  await page.goto('/');
  await dropHero(page, 'hulk', 500, 250);
  const start = await boardPoint(page, 270, 435);
  const end = await boardPoint(page, 350, 480);
