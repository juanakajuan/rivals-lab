...
test('history records and restores Deadpool role changes', () => {
  const board: BoardState = {
    mapId: DEFAULT_MAP_ID,
    tokens: [{ id: 'ally-deadpool', heroId: 'deadpool', team: 'ally', deadpoolRole: 'Duelist', x: 270, y: 435 }]
  };
  const history = boardHistoryReducer(createBoardHistory(board), {
    type: 'edit',
    update: (current) => ({
      ...current,
      tokens: current.tokens.map((token) => ({ ...token, deadpoolRole: 'Strategist' }))
    })
  });
  ...
  expect(boardHistoryReducer(undone, { type: 'redo' }).present).toEqual(history.present);
});
